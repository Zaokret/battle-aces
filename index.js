(() => {
  ...
  var unitInput = document.getElementById("select-unit");
  var cards = document.getElementsByClassName("card");
  var abilityInput = document.getElementById("select-ability");
  var abilities = document.getElementsByClassName("ability slot");
  var screenshot = document.getElementById("screenshot");
  var fileInput = document.getElementById("file-input");
  var loadBtn = document.getElementById("load-deck");
  var saveBtn = document.getElementById("save-deck");
  var downloadBtn = document.getElementById("download-deck");
  var deckSelection = document.getElementById("deck-selection");
  ...
  function handleSlotClick() {
    Array.from(cards).forEach((card) => {
      ...
      card.addEventListener("click", () => {
        ...
        unitInput.innerHTML = "";
        removeInProgress();
        const tierClasses = card.className.split(" ").filter((c) => c.indexOf("t") > -1);
        const tierIds = tierClasses.map(
          (c) => parseInt(c.replace("t", ""))
        );
    ...
    Array.from(abilities).forEach((slot) => {
      ...
      slot.addEventListener("click", () => {
        ...
        const available = units.filter((unit) => selectedUnits.includes(unit.slug)).reduce((abs, unit) => {
  ...
  function parseDeck(fileAsString) {
    const line = fileAsString.split("\n").find((line2) => line2 && line2.startsWith("decks"));
    if (line) {
      try {
  ...
  function createURL({ slugs, abilities: abilities2 }) {
    const url = new URL("https://zaokret.github.io/battle-aces/");
    url.searchParams.set("deck", window.btoa(slugs));
    url.searchParams.set("abs", window.btoa(abilities2));
    return url;
  }
  function getSlugsAndAbilities(deck) {
    const abilities2 = deck._activeAbilities.map((abi) => {
  ...
  function removeDeckPreviewBorder() {
    Array.from(document.querySelectorAll(".deck-preview")).forEach((preview) => preview.classList.remove("brighten"));
  }
  ...
  function validateAbilities(abs, selectedUnits = getSelectedUnitSlugs()) {
    ...
    const available = units.filter((unit) => selectedUnits.includes(unit.slug)).reduce((abs2, unit) => {
      if (unit.unitAbility && !abs2.includes(unit.unitAbility)) {
        abs2.push(unit.unitAbility.toLowerCase());
      }
      return abs2;
    }, []);
  ...
  function checkSelectedAbilities(selectedUnitSlugs) {
    const available = units.filter((unit) => selectedUnitSlugs.includes(unit.slug)).reduce((abs, unit) => {
      if (unit.unitAbility && !abs.includes(unit.unitAbility)) {
  ...
  function createAbilityIcon(name) {
    const url = window.location.origin + "/images/abilities/" + name.toLowerCase() + ".png";
  ...
  function removeInProgress() {
    Array.from(cards).forEach(
      (card) => card.classList.remove("selection-in-progress")
    );
  }
  ...
  function getSelectedUnitSlugs() {
    return Array.from(cards).map(
      (card) => card.hasChildNodes() ? card.children[0].id.replace(" ", "").toLowerCase() : ""
    );
  }
  ...
  function addHoverToRelatedAbilities(el) {
    el.addEventListener("mouseenter", () => {
      const slug = el.id;
      const unit = units.find((unit2) => unit2.slug === slug);
    ...
    el.addEventListener("mouseleave", () => {
      el.classList.remove("hover");
      Array.from(document.querySelectorAll(`.ability-icon`)).forEach((el2) => el2.classList.remove("hover"));
    });
  }
  function addHoverToRelatedUnits(el) {
      ...
      if (["ability-icon-selection", "filled-slot"].some((className) => el.classList.contains(className))) {
        ...
        units.forEach((unit) => {
          ...
          if (index > -1 && unit.unitAbility?.toLowerCase() === ability) {
            const el2 = document.querySelector(`.card-img#${unit.slug}`);
            if (el2) {
    ...
    el.addEventListener("mouseleave", () => {
      el.classList.remove("hover");
      Array.from(document.querySelectorAll(`.card-img`)).forEach((el2) => el2.classList.remove("hover"));
    });
  }
    ...
    const filter = createTagFilter(
      ...
      (tag, checked) => {
        ...
        unitList.forEach((unit) => {
          ...
          const included = tags.every((tag2) => {
            if (tag2 === "Anti-Air") {
              return ["Anti-Air", "Versatile"].some(
                (t) => unitTags.some((ut) => t === ut)
              );
            }
            if (tag2 === "Anti-Ground") {
              return ["Anti-Ground", "Versatile"].some(
                (t) => unitTags.some((ut) => t === ut)
              );
            }
            return unitTags.some((ut) => tag2 === ut);
          });
  ...
  function minHeight(unitList) {
    const highest = unitList.map((unit) => parseUnitTags(unit).length).sort((a, b) => b - a)[0];
    return highest * 30;
  }
  function parseUnitTags(unit) {
    return unit.unitTag.replace(" Unit", "").replace(" Damage", "").replace(" Defense", "-Defense").replace(" Range", "-Range").replace("\n", "").split(" ").concat(unit.unitAbility ? unit.unitAbility : []);
  }
  function createUnitDescription(unit, minHeight2) {
    const tags = parseUnitTags(unit);
    const desc = document.createElement("div");
    desc.className = "tag-list";
    desc.style.minHeight = minHeight2 + "px";
    tags.forEach((tag) => {
  ...
  function createStatNumber(stat) {
    ...
    for (let index = 0; index < 5; index++) {
      ...
      number.className = stat > index ? "stat-number-full" : "stat-number-empty";
  ...
  function getTierNamesByIds(ids) {
    return tiers.filter((tier) => ids.some((id) => tier.techTierId === id)).map((tier) => tier.name);
  }
  ...
  function validateDeck(names) {
    ...
    return tiers2.every((tier, index) => {
  ...
  function capture(width, height, sources, scale, crop) {
    ...
    const selectedUnits = getSelectedUnitSlugs();
    const fileName = selectedUnits.map(
      (slug) => slug ? units.find((unit) => unit.slug === slug).name : "Empty"
    );
  ...
  function fillMissing(elements) {
    if (elements.length < 8) {
      ...
      const filled = new Array(8).fill(void 0);
